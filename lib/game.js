const {Board, maxTotalShipCount, fieldStates} = require('./board.js');
...
class Game {
    ...
    move(playerNumber, obj) {
        if (this.players[playerNumber].status === playerStatus.PLACING) {
            this.#movePlace(playerNumber, obj);
        } else if (this.players[playerNumber].status === playerStatus.ATTACKING) {
            this.#moveAttack(playerNumber, obj);
        }
    }

    #movePlace(playerNumber, obj) {
    ...
    #moveAttack(playerNumber, obj) {
        ...
        let sendObj = { fieldstate, destroyedship, bomb: {x, y} };
        if (fieldstate === fieldStates.WATER) {
            this.#sendAttacking(other, sendObj);
            this.#sendIdle(playerNumber, sendObj);
        } else {
            this.#sendAttacking(playerNumber, sendObj);
            this.#sendIdle(other, sendObj);
        }
        /*
        ...
        this.#send(other, {status: playerStatus.ATTACKING, destroyedship, bomb: {x,y}});
*/
        // finish game
    ...
    #sendIdle(playerNumber, obj) {
        this.#setStatus(playerNumber, playerStatus.IDLE);
        let {fieldstate, destroyedship, bomb} = obj;
        this.#send(playerNumber, {status: playerStatus.IDLE, fieldstate, 
            destroyedship, bomb});
    }

    #sendAttacking(playerNumber, obj) {
        this.#setStatus(playerNumber, playerStatus.ATTACKING);
        let {fieldstate, destroyedship, bomb} = obj;
        this.#send(playerNumber, {status: playerStatus.ATTACKING, fieldstate, destroyedship, 
            bomb});
    }
